import FreeCAD, Part, FreeCADGui, math, os
...
sections = [
    (52.0, 44.0, -22.0, -74.0), # Bottom cap
    (52.0, 44.0, -21.8, -40.0), # Lower battery
    (52.0, 44.0, -20.2, -10.0), # Upper battery
    (51.6, 43.6, -17.2,  15.0), # Grip waist
    (51.2, 43.0, -15.0,  45.0), # Display section
    (50.8, 42.5, -13.8,  74.0), # Top cap
]

wires = [make_trapezoid_wire(*s) for s in sections]
solid = Part.makeLoft(wires, True, False)

# Longitudinal and cap edge fillets
...
# =========================================================================
# 2. TOP FACE DETAILS (IR SCOOP, DIODE, 3 STEPPED SLITS)
# =========================================================================
top_lip_wire = make_trapezoid_wire(47.5, 39.0, -13.0, 74.0)
top_lip_face = Part.Face(top_lip_wire)
try:
    top_lip_face = top_lip_face.makeFillet(3.0, [top_lip_face.Vertexes[0], top_lip_face.Vertexes[1]])
except Exception:
    pass
top_lip_cut = top_lip_face.extrude(FreeCAD.Vector(0, -0.8, 0))
body = body.cut(top_lip_cut)
...
# =========================================================================
# 6. BACK SIDE: HOUSING WITH TOP HALF-CIRCLE & INTERNAL BATTERY BAY
# =========================================================================
BAT_W = 31.0
BAT_L = 67.0
BAT_Y_START = -74.0
BAT_Y_END = -7.0 # Seam line where cover meets enclosure

# 1. Taper-aligned, Uniform-Depth Rounded Rectangle Label Pocket
# Width = 14.0 mm, Height (along Y) = 18.0 mm, Corner Radius = 2.8 mm
# Center Y = 32.0 mm, Uniform Depth = 0.55 mm
RECT_CY = 32.0
RECT_W = 14.0
RECT_H = 18.0
RECT_R = 2.8
...
xc = RECT_W/2.0 - RECT_R
yc = RECT_H/2.0 - RECT_R
k = RECT_R * math.sqrt(2)/2.0

p_tr_s = FreeCAD.Vector(xc + RECT_R, yc, 0)
p_tr_m = FreeCAD.Vector(xc + k, yc + k, 0)
p_tr_e = FreeCAD.Vector(xc, yc + RECT_R, 0)

p_tl_s = FreeCAD.Vector(-xc, yc + RECT_R, 0)
p_tl_m = FreeCAD.Vector(-xc - k, yc + k, 0)
p_tl_e = FreeCAD.Vector(-xc - RECT_R, yc, 0)

p_bl_s = FreeCAD.Vector(-xc - RECT_R, -yc, 0)
p_bl_m = FreeCAD.Vector(-xc - k, -yc - k, 0)
p_bl_e = FreeCAD.Vector(-xc, -yc - RECT_R, 0)

p_br_s = FreeCAD.Vector(xc, -yc - RECT_R, 0)
p_br_m = FreeCAD.Vector(xc + k, -yc - k, 0)
p_br_e = FreeCAD.Vector(xc + RECT_R, -yc, 0)

e_top = Part.makeLine(p_tr_e, p_tl_s)
arc_tl = Part.Arc(p_tl_s, p_tl_m, p_tl_e).toShape()
e_left = Part.makeLine(p_tl_e, p_bl_s)
arc_bl = Part.Arc(p_bl_s, p_bl_m, p_bl_e).toShape()
e_bot = Part.makeLine(p_bl_e, p_br_s)
arc_br = Part.Arc(p_br_s, p_br_m, p_br_e).toShape()
e_right = Part.makeLine(p_br_e, p_tr_s)
arc_tr = Part.Arc(p_tr_s, p_tr_m, p_tr_e).toShape()

wire_2d = Part.Wire([e_top, arc_tl, e_left, arc_bl, e_bot, arc_br, e_right, arc_tr])
face_2d = Part.Face(wire_2d)

solid_pocket = face_2d.extrude(FreeCAD.Vector(0, 0, 3.0))
floor_edges = [e for e in solid_pocket.Edges if abs(e.Vertexes[0].Point.z - 3.0) < 0.05 and abs(e.Vertexes[1].Point.z - 3.0) < 0.05]
try:
    solid_pocket = solid_pocket.makeFillet(0.3, floor_edges)
except Exception:
    pass

solid_pocket.translate(FreeCAD.Vector(0, 0, - (3.0 - RECT_DEPTH)))

slope = 2.2 / 30.0
theta_deg = math.degrees(math.atan(slope))
solid_pocket.rotate(FreeCAD.Vector(0, 0, 0), FreeCAD.Vector(1, 0, 0), -theta_deg)

Z_surf_center = -17.2 + (RECT_CY - 15.0) * slope
solid_pocket.translate(FreeCAD.Vector(0.0, RECT_CY, Z_surf_center))

body = body.cut(solid_pocket)

# 2. Main Battery Bay Opening Cutout in Remote Enclosure
bay_opening_box = Part.makeBox(BAT_W, BAT_L + 0.5, 5.0)
bay_opening_box.translate(FreeCAD.Vector(-BAT_W/2.0, -74.5, -23.5))
body = body.cut(bay_opening_box)

# 3. Internal Deep Battery Cavity (2x AAA slots with center divider rib)
...
# 4. Internal Latch Slot inside body top ceiling (hidden internally)
latch_slot = Part.makeBox(6.5, 3.5, 1.8)
latch_slot.translate(FreeCAD.Vector(-3.25, -9.5, -16.8))
body = body.cut(latch_slot)

# 5. Shared Spherical Thumb Cutter (creates unified seamless circle dish across parting seam)
R_sph = 16.0
Z_center = -19.84 + 0.85 - R_sph
thumb_cutter = Part.makeSphere(R_sph, FreeCAD.Vector(0.0, BAT_Y_END, Z_center))

# Cut top half on body
body = body.cut(thumb_cutter)

# 6. Alignment nibs on body
nib1 = Part.makeSphere(0.65)
nib1.translate(FreeCAD.Vector(-7.5, BAT_Y_END + 1.2, -19.6))
nib2 = Part.makeSphere(0.65)
...
# =========================================================================
# 7. SEPARATE BATTERY COVER WITH BOTTOM HALF-CIRCLE & SLIDE ARROW
# =========================================================================
cover_bound = Part.makeBox(BAT_W - 0.2, BAT_L, 4.0)
cover_bound.translate(FreeCAD.Vector(-(BAT_W - 0.2)/2.0, -74.0, -23.0))
cover_raw = body_base.common(cover_bound)

# Hollow out ONLY battery bay zone, keeping thumb scoop area solid
cover_inner_cut = Part.makeBox(BAT_W - 3.0, 59.0, 3.0)
cover_inner_cut.translate(FreeCAD.Vector(-(BAT_W - 3.0)/2.0, -73.0, -19.5))
cover_shell = cover_raw.cut(cover_inner_cut)

# Cut bottom half of thumb scoop on cover
cover_shell = cover_shell.cut(thumb_cutter)

# Internal Latch Hook Tab (UNDERNEATH ONLY, purely internal, hidden from exterior)
hook_tongue = Part.makeBox(5.0, 4.0, 1.2)
hook_tongue.translate(FreeCAD.Vector(-2.5, BAT_Y_END - 1.0, -16.2))
hook_barb = Part.makeBox(5.0, 1.2, 0.7)
hook_barb.translate(FreeCAD.Vector(-2.5, BAT_Y_END + 1.8, -15.7))
hook_full = hook_tongue.fuse(hook_barb)

# Underside battery ribs
rib1 = Part.makeBox(1.2, 34.0, 1.8)
rib1.translate(FreeCAD.Vector(-6.5, -57.0, -19.8))
rib2 = Part.makeBox(1.2, 34.0, 1.8)
rib2.translate(FreeCAD.Vector(5.3, -57.0, -19.8))

# Engraved slide arrow '▼' on the bottom half-circle (shallow, clean mark)
p1 = FreeCAD.Vector(-1.6, BAT_Y_END - 2.2, -18.7)
p2 = FreeCAD.Vector( 1.6, BAT_Y_END - 2.2, -18.7)
p3 = FreeCAD.Vector( 0.0, BAT_Y_END - 4.4, -18.7)
arrow_wire = Part.makePolygon([p1, p2, p3, p1])
arrow_cut = Part.Face(arrow_wire).extrude(FreeCAD.Vector(0, 0, -0.6))

# Dual Resting Support Feet on cover lower exterior
...
if FreeCADGui.ActiveDocument:
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/back_closed_clean.png', 1200, 1600, 'Current')
    
    # 2. Back dish close-up
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.zoomIn()
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_closed_clean.png', 1200, 1600, 'Current')
    
    # 4. Back view open (cover hidden)
    gui_cover.Visibility = False
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.saveImage(f'{art_dir}/back_open_clean.png', 1200, 1600, 'Current')
    
    # 5. Iso view open
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_open_clean.png', 1200, 1600, 'Current')
    
    # 6. Front view
    gui_cover.Visibility = True
    view.setCameraOrientation(FreeCAD.Rotation(0.0, 0.0, 0.0, 1.0))
    view.fitAll()
    view.saveImage(f'{art_dir}/front_view_final.png', 1200, 1600, 'Current')
