...
body = body_base.cut(ellip_scoop)
...
# 1. Label recess
label_box = Part.makeBox(16.0, 22.0, 1.0)
label_box.translate(FreeCAD.Vector(-8.0, 17.0, -16.5))
v_e = [e for e in label_box.Edges if abs(e.Length - 1.0) < 0.05]
label_box = label_box.makeFillet(3.0, v_e)
body = body.cut(label_box)

# 2. Main Battery Bay Opening Cutout in Remote Enclosure
# Cuts from Y=-74.5 to Y=-7.0, width BAT_W, from Z=-24.0 up to inside Z=-18.0
bay_opening_box = Part.makeBox(BAT_W, BAT_L + 0.5, 6.0)
...
# 4. Internal Latch Slot in body top wall
latch_slot = Part.makeBox(6.5, 3.5, 1.8)
latch_slot.translate(FreeCAD.Vector(-3.25, -9.5, -16.8))
body = body.cut(latch_slot)

# 5. Continuous Thumb Dish Cutter (R=16.0, centered so it cuts ~0.9mm deep into outer surface)
# Back surface at (0, -7.0) is at Z = -19.84.
R_sph = 16.0
Z_center = -19.84 + 0.90 - R_sph # -34.94
...
# 7. BATTERY COVER SOLID (Constructed directly from body_base so exterior is 100% FLUSH)
cover_bound = Part.makeBox(BAT_W - 0.2, BAT_L, 4.0)
cover_bound.translate(FreeCAD.Vector(-(BAT_W - 0.2)/2.0, -74.0, -23.0))
# Intersect body_base with cover_bound to get the exact matching curved back shell
cover_raw = body_base.common(cover_bound)

# Hollow out the inside of the cover so it has 1.8mm shell thickness
cover_inner_cut = Part.makeBox(BAT_W - 3.0, BAT_L - 1.0, 3.0)
cover_inner_cut.translate(FreeCAD.Vector(-(BAT_W - 3.0)/2.0, -73.5, -19.5))
...
p1 = FreeCAD.Vector(-1.8, BAT_Y_END - 2.2, -18.7)
p2 = FreeCAD.Vector( 1.8, BAT_Y_END - 2.2, -18.7)
p3 = FreeCAD.Vector( 0.0, BAT_Y_END - 4.6, -18.7)
...
if FreeCADGui.ActiveDocument:
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/back_closed_flush.png', 1200, 1600, 'Current')
    
    # Close-up on the circle dish
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.zoomIn()
    view.zoomIn()
    view.saveImage(f'{art_dir}/back_dish_flush_closeup.png', 1200, 1200, 'Current')
    
    # Iso view closed
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_closed_flush.png', 1200, 1600, 'Current')
    
    # Back view open (cover hidden)
    gui_cover.Visibility = False
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.saveImage(f'{art_dir}/back_open_flush.png', 1200, 1600, 'Current')
    
    # Iso view open
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_open_flush.png', 1200, 1600, 'Current')

doc.saveAs('/tmp/freecad/LG_AC_Remote.FCStd')
print('FLUSH TEST COMPLETE')
'''
