...
# 1. Label recess (rounded rectangle, shallow 0.5mm depth)
label_pocket = make_rounded_rect_btn(14.0, 22.0, 1.0, 2.5, 0.0, 26.0, z_base=-16.6)
...
# 4. Internal Latch Slot inside body ceiling (Z = -16.5 to -15.0)
latch_slot = Part.makeBox(6.5, 3.5, 1.8)
latch_slot.translate(FreeCAD.Vector(-3.25, -9.5, -16.8))
body = body.cut(latch_slot)

# 5. Shared Spherical Thumb Cutter (creates unified circle dish)
R_sph = 16.0
...
# Hollow out ONLY the battery bay zone (Y = -73.0 to -14.0), keeping Y = -14.0 to -7.0 solid 2.0mm!
...
if FreeCADGui.ActiveDocument:
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/back_closed_perfect.png', 1200, 1600, 'Current')
    ...
    view.zoomIn()
    view.zoomIn()
    view.saveImage(f'{art_dir}/back_dish_perfect_closeup.png', 1200, 1200, 'Current')
    
    # Iso view closed
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_closed_perfect.png', 1200, 1600, 'Current')
    
    # Back view open (cover hidden)
    gui_cover.Visibility = False
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.saveImage(f'{art_dir}/back_open_perfect.png', 1200, 1600, 'Current')
    
    # Iso view open
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_open_perfect.png', 1200, 1600, 'Current')

doc.saveAs('/tmp/freecad/LG_AC_Remote.FCStd')
print('PERFECT TEST COMPLETE')
'''
