...
# Width = 15.0 mm, Height (along Y) = 17.0 mm, Corner Radius = 3.0 mm
# Center Y = 32.0 mm, Uniform Depth = 0.60 mm
RECT_CY = 32.0
RECT_W = 15.0
RECT_H = 17.0
RECT_R = 3.0
RECT_DEPTH = 0.60
H_BOX = 4.0 # Height of cutter box

# Create cutter solid in local frame
pocket_box = Part.makeBox(RECT_W, RECT_H, H_BOX)
pocket_box.translate(FreeCAD.Vector(-RECT_W/2.0, -RECT_H/2.0, - (H_BOX - RECT_DEPTH)))
v_e = [e for e in pocket_box.Edges if abs(e.Length - H_BOX) < 0.05]
pocket_solid = pocket_box.makeFillet(RECT_R, v_e)
...
if FreeCADGui.ActiveDocument:
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/back_closed_tweaked_rect.png', 1200, 1600, 'Current')
    
    # Close-up of the upper back section with rectangle and dish
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.zoomIn()
    view.saveImage(f'{art_dir}/back_upper_tweaked_closeup.png', 1200, 1200, 'Current')
    
    # Iso view
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_tweaked_rect.png', 1200, 1600, 'Current')

doc.saveAs('/tmp/freecad/LG_AC_Remote.FCStd')
print('TWEAKED RECT TEST COMPLETE')
'''
