...
body = body.cut(slot1.fuse(slot2).fuse(slot3))
...
# Pocket parameters:
# Center Y = 32.0 mm
# Width = 15.0 mm, Height (Y) = 17.0 mm, Corner Radius = 3.5 mm
# Constant Depth = 0.65 mm
RECT_CY = 32.0
RECT_W = 15.0
RECT_H = 17.0
RECT_R = 3.5
...
# Surface slope angle: between Y=15 (Z=-17.2) and Y=45 (Z=-15.0)
# slope = ( -15.0 - (-17.2) ) / 30.0 = 2.2 / 30.0 = 0.073333
slope = 2.2 / 30.0
theta_deg = math.degrees(math.atan(slope)) # ~4.194 deg
theta_rad = math.atan(slope)

# Surface Z at RECT_CY:
Z_surf_center = -17.2 + (RECT_CY - 15.0) * slope # -17.2 + 17.0 * 0.07333 = -15.953 mm

# Construct rounded rectangle wire in local frame (origin at center)
def make_rounded_rect_wire(w, l, r):
    dx = w/2.0 - r
    dy = l/2.0 - r
    p1 = FreeCAD.Vector(dx, dy + r, 0)
    p2 = FreeCAD.Vector(-dx, dy + r, 0)
    p3 = FreeCAD.Vector(-dx - r, dy, 0)
    p4 = FreeCAD.Vector(-dx - r, -dy, 0)
    p5 = FreeCAD.Vector(-dx, -dy - r, 0)
    p6 = FreeCAD.Vector(dx, -dy - r, 0)
    p7 = FreeCAD.Vector(dx + r, -dy, 0)
    p8 = FreeCAD.Vector(dx + r, dy, 0)
    
    e_top = Part.makeLine(p1, p2)
    arc_tl = Part.Arc(p2, FreeCAD.Vector(-dx - r*(1-math.cos(math.pi/4)), dy + r*(1-math.sin(math.pi/4)), 0), p3).toShape()
    e_left = Part.makeLine(p3, p4)
    arc_bl = Part.Arc(p4, FreeCAD.Vector(-dx - r*(1-math.cos(math.pi/4)), -dy - r*(1-math.sin(math.pi/4)), 0), p5).toShape()
    e_bot = Part.makeLine(p5, p6)
    arc_br = Part.Arc(p6, FreeCAD.Vector(dx + r*(1-math.cos(math.pi/4)), -dy - r*(1-math.sin(math.pi/4)), 0), p7).toShape()
    e_right = Part.makeLine(p7, p8)
    arc_tr = Part.Arc(p8, FreeCAD.Vector(dx + r*(1-math.cos(math.pi/4)), dy + r*(1-math.sin(math.pi/4)), 0), p1).toShape()
    
    return Part.Wire([e_top, arc_tl, e_left, arc_bl, e_bot, arc_br, e_right, arc_tr])

rect_wire = make_rounded_rect_wire(RECT_W, RECT_H, RECT_R)
rect_face = Part.Face(rect_wire)

# Extrude along local +Z by (RECT_DEPTH + 2.0)
# Then translate so the base is at Z = -2.0, top is at Z = RECT_DEPTH
pocket_solid = rect_face.extrude(FreeCAD.Vector(0, 0, RECT_DEPTH + 2.0))
pocket_solid.translate(FreeCAD.Vector(0, 0, -2.0))

# Rotate solid around X axis by theta_deg so normal tilts to match back face slope
pocket_solid.rotate(FreeCAD.Vector(0, 0, 0), FreeCAD.Vector(1, 0, 0), -theta_deg)

# Translate to target position (0.0, RECT_CY, Z_surf_center)
pocket_solid.translate(FreeCAD.Vector(0.0, RECT_CY, Z_surf_center))

# Cut uniform depth rounded rectangle pocket into body
body = body.cut(pocket_solid)
...
# 3. Internal Deep Battery Cavity (2x AAA slots)
...
# Internal Latch Hook on underside of cover
...
# Dual Resting Feet on cover bottom exterior
...
if FreeCADGui.ActiveDocument:
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/back_closed_uniform_rect.png', 1200, 1600, 'Current')
    
    # Close-up of the upper back section with rectangle and dish
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.zoomIn()
    view.saveImage(f'{art_dir}/back_upper_closeup.png', 1200, 1200, 'Current')
    
    # Iso view
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_uniform_rect.png', 1200, 1600, 'Current')

doc.saveAs('/tmp/freecad/LG_AC_Remote.FCStd')
print('UNIFORM RECT TEST COMPLETE')
'''
