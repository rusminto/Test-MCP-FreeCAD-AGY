...
# 1. 100% Symmetric, Uniform-Depth Rounded Rectangle Label Pocket
RECT_CY = 32.0
RECT_W = 15.0
RECT_H = 17.0
RECT_R = 3.0
...
# Build exact 2D wire
xc = RECT_W/2.0 - RECT_R # 4.5
yc = RECT_H/2.0 - RECT_R # 5.5
...
# Extrude by total 3.0mm (from -2.0 to +1.0 relative to plane)
...
if FreeCADGui.ActiveDocument:
    ...
    view.fitAll()
    view.saveImage(f'{art_dir}/back_closed_symmetric_rect.png', 1200, 1600, 'Current')
    
    # Close-up of the upper back section with rectangle and dish
    view.setCameraOrientation(FreeCAD.Rotation(FreeCAD.Vector(0, 1, 0), 180))
    view.fitAll()
    view.zoomIn()
    view.saveImage(f'{art_dir}/back_upper_symmetric_closeup.png', 1200, 1200, 'Current')
    
    # Iso view
    view.setCameraOrientation(FreeCAD.Rotation(0.24, -0.74, 0.60, 0.18))
    view.fitAll()
    view.saveImage(f'{art_dir}/iso_symmetric_rect.png', 1200, 1600, 'Current')

doc.saveAs('/tmp/freecad/LG_AC_Remote.FCStd')
print('SYMMETRIC RECT TEST COMPLETE')
'''
